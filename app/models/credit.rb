class Credit
  ...
  attr_accessor :postal_code,:city,:house_number,:building_name,:phone_number,:prefecture_id,:user_id,:order_id
  with_options presence: true do
    validates :postal_code, format: { with: /\A[0-9]{3}-[0-9]{4}\z/}
    validates :city,format: { with: /\A[ぁ-ん一-龥々]+\z/}
    validates :house_number,format:{with:/\A[ぁ-んァ-ヶ一-龥々]+\z/}
    validates :building_name,format: { with: /\A[ぁ-んァ-ヶ一-龥々]+\z/}
